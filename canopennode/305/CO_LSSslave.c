            ...
            switch (mode) {
                case CO_LSS_STATE_WAITING:
                    if ((LSSslave->lssState == CO_LSS_STATE_CONFIGURATION)
                        && (*LSSslave->pendingNodeID != LSSslave->activeNodeID)) {
            ...
            switch (cs) {
                ...
                case CO_LSS_IDENT_FASTSCAN: {
                    ...
                    if (ack) {
                        ...
                        memset(&LSSslave->TXbuff->data[1], 0, sizeof(LSSslave->TXbuff->data) - 1U);
...
CO_LSSslave_process(CO_LSSslave_t* LSSslave) {
    ...
    if (CO_FLAG_READ(LSSslave->sendResponse)) {
        ...
        switch (LSSslave->service) {
            ...
            case CO_LSS_INQUIRE_BITRATE: {
                uint16_t bitRate = (LSSslave->pendingBitRate != NULL) ? *LSSslave->pendingBitRate : 0U;
                LSSslave->TXbuff->data[0] = LSSslave->service;
                LSSslave->TXbuff->data[1] = (uint8_t)(bitRate & 0xFFU);
                LSSslave->TXbuff->data[2] = (uint8_t)((bitRate >> 8) & 0xFFU);
                CANsend = true;
                break;
            }
            case CO_LSS_IDENT_FASTSCAN: {
