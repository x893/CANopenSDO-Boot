                ...
                case CO_SDO_ST_DOWNLOAD_SEGMENT_REQ: {
                    if ((SDO->CANrxData[0] & 0xE0U) == 0x00U) {
                        ...
                        if (toggle != SDO->toggle) {
                            uint8_t prevToggle = (SDO->toggle == 0x00U) ? 0x10U : 0x00U;
                            if (toggle == prevToggle) {
                                /* Duplicate segment request: re-send last ack without re-writing OD data. */
                                SDO->CANtxBuff->data[0] = 0x20U | toggle;
                                SDO->timeoutTimer = 0;
                ...
                case CO_SDO_ST_UPLOAD_SEGMENT_REQ: {
                    if ((SDO->CANrxData[0] & 0xEFU) == 0x60U) {
                        ...
                        if (toggle != SDO->toggle) {
                            uint8_t prevToggle = (SDO->toggle == 0x00U) ? 0x10U : 0x00U;
                            if (toggle == prevToggle) {
                                /* Duplicate upload request: re-send last upload segment response. */
                                SDO->timeoutTimer = 0;
                                (void)CO_CANsend(SDO->CANdevTx, SDO->CANtxBuff);
