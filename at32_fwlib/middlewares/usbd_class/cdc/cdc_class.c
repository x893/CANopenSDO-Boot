...
void usb_usart_config(register linecoding_type *linecoding);
void usb_usart_set_line_state(register uint16_t data);
...
static usb_sts_type class_setup_handler(void *udev, usb_setup_type *setup)
{
	...
	case USB_REQ_TYPE_CLASS:
		...
		{
			usb_vcp_cmd_process(udev, setup->bRequest, (uint8_t *)&setup->wValue, setup->wLength);
			usbd_ctrl_send(pudev, pcdc->g_cmd, setup->wLength);
		}
		break;
	/* standard request */
...
/**
  ...
  */
uint16_t usb_vcp_get_rxdata(void *udev, uint8_t *recv_data, uint16_t max_length)
{
	uint16_t i_index;
	uint16_t tmp_len = 0;
	usbd_core_type *pudev = (usbd_core_type *)udev;
	...
	for(i_index = 0; i_index < tmp_len; i_index++)
	{
...
static void usb_vcp_cmd_process(void *udev, uint8_t cmd, uint8_t *buff, uint16_t len)
{
	...
	case SET_CONTROL_LINE_STATE:
		if (len == 0)
			usb_usart_set_line_state(buff[0]);
		break;
	default:
		break;
	}
